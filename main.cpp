...
class ExampleRTThread : public CyclicThread {
 ...
 protected:
  /**
   * @brief This methods runs every loop, which for this particular example is
   * every 1ms.
   *
   * @param elapsed_ns The number of nanoseconds elapsed since the App::Start
   * was called.
  ...
  LoopControl Loop(int64_t elapsed_ns) noexcept final {
    ...
    // This demonstrates the usage of the quill logger. This emits a log message
    // every 1s.
 ...
 private:
  ...
  static cactus_rt::CyclicThreadConfig MakeConfig() {
    ...
    // Pin this thread on CPU core #2. If you don't want to pin in on a CPU
    // core, remove the following line.
...
int main() {
  ...
  std::cout << "Tracing available: " << cactus_rt::tracing::IsTracingAvailable() << "\n";
